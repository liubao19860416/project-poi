...
function yycgdsave(){
	...
	$("#yycgdsaveForm").attr("action","${baseurl}cgd/yycgdeditsubmit.action");
...
function yycgdsaveas(){
	//form提交，执行另存为操作
	_confirm('您确定要将此采购单另存为一个新的采购单吗?',null,
		function(){
			$("#yycgdsaveForm").attr("action","${baseurl}cgd/yycgdcopy.action");
			jquerySubByFId('yycgdsaveForm', yycgdsaveas__callback, null);
		}
	);
}
function yycgdsaveas__callback(data){
	var result = getCallbackData(data);
	_alert(result);
	//如果另存为成功则跳转到新的采购单修改页面
	//在这里必须拿新添加的 采购单的id
	 if(result.type==TYPE_RESULT_SUCCESS){//如果成功跳转至修改页面
		//alert(result.sysdata.yycgdid);
		var yycgdid=data.resultInfo.sysdata.yycgdid;
		//新开一个窗口在新窗口中展示新生成的采购单信息
		parent.opentabwindow(yycgdid+'采购单修改','${baseurl}cgd/yycgdedit.action?yycgdid='+yycgdid);//打开一个新标签
	} 
}
...
function yycgdsubmit_callback(data) {
	...
	if(result.type==TYPE_RESULT_SUCCESS){
		window.location="${baseurl}cgd/yycgdedit.action?yycgdid=${yycgd.id}";
	}
...
function yycgdmxaddshow(){
	var sendUrl="${baseurl}cgd/yycgdmxaddquery.action?yycgdid=${yycgd.id}";
	...
	_confirm('您确定要保存选择的采购药品吗?',null,
			  function(){
				...
				if(rows.length>0){
					...
					$("#yycgdmxForm").attr("action",'${baseurl}cgd/yycgdmxsavesubmit.action');
...
<c:if test="${yycgd.zt=='1' || yycgd.zt=='4'}">
toolbar = [{
	id : 'yycgdmxaddshow',
	text : '采购药品添加',
	iconCls : 'icon-add',
...
var columns = [ [{
	...
	formatter: function(value,row,index){
		if(value){
			return '<input type="hidden" name="yycgdmxs['+index+'].ypxxid" value="'+row.id+'" />';
		}
...
//加载datagrid（yycgdmxlist）
	 ...
	 $(function() {
		$('#yycgdmxlist').datagrid({
			...
			striped : true,
			url : '${baseurl}cgd/yycgdmxlist.action',//这里边后边带了一个参数，所以form中不需要此参数yycgdid
			 queryParams:{//url的参数，初始加载datagrid时使用的参数
				yycgdid:'${yycgd.id}'//yycgdid是参数名称，如果参数名称中间有点，将参数用单引号括起来
			}, 
					...
					<TABLE class="toptable grid" border=1 cellSpacing=1 cellPadding=4
						...
						<TBODY>
							...
							<TR>
								...
								<c:if test="${yycgd.zt=='2' || yycgd.zt=='3' }">
								${yycgd.mc}
								</c:if>
								</div>
...
<TABLE border=0 cellSpacing=0 cellPadding=0 width="100%" bgColor=#c4d8ed>
	...
	<input type="hidden" name="yycgdid" value="${yycgd.id}"/>
