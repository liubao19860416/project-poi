...
var yycgdmxadd = function(){
	_confirm('您确定要添加选中的药品吗?',null,
	  function(){
		...
		if(rows.length>0){
			...
			//执行ajax的form提交
			jquerySubByFId('gysypmlForm', yycgdmxadd_callback, null);
		}else{
...
function yycgdmxadd_callback(data) {
...
var columns = [ [{
	...
	formatter: function(value,row,index){
		//gysypmls对应action接收对象中list的名称，[]括号中是从0开始序号,ypxxid是list中对象属性
	...
	formatter: function(value,row,index){
		//gysypmls对应action接收对象中list的名称，[]括号中是从0开始序号,ypxxid是list中对象属性
		return '<input type="hidden" name="yycgdmxs['+index+'].usergysid" value="'+value+'" />';
	}
},{
	field : 'usergysmc',
	title : '供货企业',
...
function initGrid(){
	$('#gysypmllist').datagrid({
		...
		striped : true,
		//collapsible : true,
		url : '${baseurl}cgd/yycgdmxaddquery_result.action',
		...
		//sortOrder : 'desc',
		//remoteSort : false,
		idField : 'gysypmlid',//json数据集的主键(药品信息id)
