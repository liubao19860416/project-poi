...
var columns = [ [{
	...
	field : 'id',
	hidden:true,
	formatter:function(value, row, index){//index是每行的序号
		return '<input type="hidden" name="gysypmls['+index+'].ypxxid" id="'+row.id+'" value="'+value+'" />';
	}
},{
	field : 'bm',
	title : '流水号',
...
var dataGrid_obj;//datagrid的对象
	...
	$(function() {
		...
		dataGrid_obj.datagrid({
			...
			//将点击行执行取该行选中，因为jqueryeasyui在点击行的时候默认选中行
...
<div id="ypxxquery_div">
    <form id="gysypmladdqueryForm" name="gysypmladdqueryForm" action="${baseurl}ypml/gysypmladdsubmit.action" method="post">
